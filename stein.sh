...
READ_LENGTH=101
MAP_QUALITY_FILTER=0
MAP_QUALITY_DEFAULT=0
BASE_QUALITY="F"
SORT=true
INCOMPLETE_N=false
THREADS=4
...
# Loop through files in the input directory and process them
INPUT_DIR="test"
OUTPUT_DIR="test"
# for INPUT in $(ls ${INPUT_DIR}/*.bgz | sort -r);
for INPUT in ${INPUT_DIR}/*.bgz;
do
    ...
    # Check if the output file already exists, if so, skip this step
    if [[ -f "${OUTPUT}" ]]; then
    ...
    echo "Creating paired reads and add fragments length into a temp BED file ${TMPBED} ..."
    ...
    bamCoverage -b "${OUTPUT}" -o "${OUTPUT_BW}" -p 8
